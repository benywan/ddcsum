...
from ddcsum.util import user_dir

log = logging.getLogger(__name__)

SYSTEM_CONFIG_PATH = "/etc/ddcsum.conf"
...
DEFAULT_CONFIG = {
    'default_servers': {
        'lbryum8.lbry.io': {'t': '50001'},
        'lbryum9.lbry.io': {'t': '50001'},
    },
    'chain': 'lbrycrd_main'
}
...
class SimpleConfig(object):
    ...
    def __init__(self, options={}, read_system_config_function=None,
        ...
        self.user_config = {}  # for self.get in ddcsum_path()
        self.path = self.ddcsum_path()
    ...
    def ddcsum_path(self):
        # Read ddcsum_path from command line / system configuration
        # Otherwise use the user's default data directory.
        path = self.get('ddcsum_path')
        if path is None:
            path = self.user_dir()
        ...
        log.info("ddcsum directory: %s", path)
        return path
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "ddcsum.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/ddcsum.conf."""
    result = {}
...
def read_user_config(path):
    """Parse and store the user config settings in ddcsum.conf into user_config[]."""
    if not path:
        return {}
